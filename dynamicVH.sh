...
function main()
{
    ...
    read SERVER_NAME
    if [ $SERVER_NAME="" ]
    	then SERVER_NAME='admin@example.com'
    fi

    echo "Enter ServerAlias: (ex: example.local)"
    read SERVER_ALIAS
    echo "Enter DocumentRoot: (ex: /var/www/html/myProject)"
    read DOC_ROOT
    ...
    echo $IP_VH
    echo "Enter absolute path of your project: (ex: /home/users/projects/myproject)"
    read PATH_PROJECT
    ...
    ln -s $PATH_PROJECT /var/www/html/

    writeHostsFile


    a2ensite ${VH_CONF} >/dev/null 2>&1 
